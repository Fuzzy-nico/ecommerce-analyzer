...
if st.button("Analizza Sito") and url:
    try:
        ...
        if load_time > 3:
            problemi.append(f"Sito lento ({load_time}s)")
            motivi.append("Esperienza utente negativa")
            punteggio -= 10
        if not soup.find("footer"):
            problemi.append("Footer mancante")
            motivi.append("Mancano i riferimenti aziendali")
            punteggio -= 10
        if not soup.find_all("h1"):
            problemi.append("Manca H1")
            motivi.append("Sito poco strutturato")
            punteggio -= 5

        servizio = "💡 Intervento Tecnico" if ruolo == "Hunter" else "💼 Restyling Avanzato"
