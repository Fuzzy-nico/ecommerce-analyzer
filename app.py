...
if not st.session_state.logged_in:
    ...
    st.markdown("---")
    if st.button(CLOSER_BTN):
        choice = st.selectbox("Da quale prodotto front-end proviene il cliente?", ["Strategico","Tecnica","Content"])
        if choice == "Strategico":
            opt = st.radio("Quale obiettivo?", ["Vendita","Incrementare volume","Moltiplicare"])
            mapping = {"Vendita":"Start: €1.200","Incrementare volume":"Top: €1.500","Moltiplicare":"Performance: €2.000"}
            st.success(mapping.get(opt))
        elif choice == "Tecnica":
            opt = st.radio("Quale obiettivo?", ["Creazione sito","Vendere","Incrementare volume"])
            mapping = {"Creazione sito":"Restyling: €1.500","Vendere":"Start: €1.200","Incrementare volume":"Top: €1.500"}
            st.success(mapping.get(opt))
        else:
            opt = st.radio("Quale obiettivo?", ["Visibilità","Vendere","Incrementare volume"])
