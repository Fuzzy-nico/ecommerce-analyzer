...
with col1:
    ...
    st.subheader("🔑 Closer")
    prod = st.selectbox("Da quale pacchetto front-end proviene il cliente?", ["Strategico","Tecnica","Social"], key="closer_prod")
    if prod=="Strategico":
        obj = st.selectbox("Quale obiettivo?", ["Vendita","Incrementare volume","Moltiplicare"], key="closer_obj")
        if obj=="Vendita":
        ...
        obj = st.selectbox("Quale obiettivo?", ["Creazione sito","Vendere","Incrementare volume"], key="closer_obj2")
        ...
        obj = st.selectbox("Quale obiettivo?", ["Visibilità","Vendere","Incrementare volume"], key="closer_obj3")
        if obj=="Visibilità":
...
with col2:
    ...
    url = st.text_input("Inserisci URL (con http/https)", "", key="url_input")
    if st.button("🚀 Avvia Analisi", key="analyse_btn"):
        if not url.startswith("http"):
            ...
            try:
                ...
                if not soup.find(lambda t: t.name in ["button","a"] and "buy" in t.text.lower()):
                    probs.append("⚠️ Nessuna CTA chiara")
                if not soup.find(class_="review"):
                    probs.append("❌ Nessuna recensione")
                footer = soup.find("footer")
                if not footer:
                    probs.append("❌ Footer mancante")
                else:
                    if "privacy" not in footer.text.lower():
                        probs.append("⚠️ Footer senza Privacy")
                if not soup.find(text=lambda t: t and "chi siamo" in t.lower()):
                    probs.append("⚠️ Manca sezione “Chi siamo”")
                if not any(k in soup.text.lower() for k in ["reso","garanzia","spedizione"]):
                    probs.append("⚠️ Manca badge fiducia")
