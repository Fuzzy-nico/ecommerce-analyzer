...
if st.button("Analizza Sito") and url:
    try:
        ...
        punteggio = penalizza(not soup.find(string=lambda x: x and "blog" in x.lower()), "⚠️ Nessun blog o contenuto", "Manca contenuto per SEO", 5, punteggio)
        punteggio = penalizza(not soup.find(string=lambda x: x and "garanzia" in x.lower()), "⚠️ Nessuna garanzia o badge fiducia", "Manca rassicurazione per l’utente", 10, punteggio)
        ...
        # Pagina prodotto simulata (prima pagina con 'product' nell'URL o simile link)
        prodotto_url = None
        for link in soup.find_all("a", href=True):
            href = link['href']
            if "product" in href or "prodott" in href or "shop" in href:
                if href.startswith("http"):
                    prodotto_url = href
                else:
                    prodotto_url = url.rstrip("/") + "/" + href.lstrip("/")
                break

        if prodotto_url:
            try:
                prod_page = requests.get(prodotto_url, timeout=10)
                prod_soup = BeautifulSoup(prod_page.text, "html.parser")
                punteggio = penalizza(not prod_soup.find(string=lambda x: x and "descrizion" in x.lower()), "⚠️ Prodotto senza descrizione efficace", "Non aiuta la vendita", 10, punteggio)
                punteggio = penalizza(not prod_soup.find(string=lambda x: x and "recension" in x.lower()), "⚠️ Prodotto senza recensioni", "Nessuna prova sociale", 10, punteggio)
                punteggio = penalizza(not prod_soup.find(string=lambda x: x and "garanzia" in x.lower()), "⚠️ Prodotto senza rassicurazioni", "Cliente non si sente protetto", 10, punteggio)
            except:
                problemi.append("⚠️ Pagina prodotto non accessibile")
                motivi.append("Impossibile analizzare contenuti della scheda")
