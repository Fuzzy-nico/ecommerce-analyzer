...
if st.button("Analizza Sito") and url:
    try:
        ...
        def penalizza(condizione, problema, motivo, punti):
            nonlocal punteggio
            if condizione:
        ...
        penalizza(load_time > 3, "⚠️ Sito lento", "Esperienza utente negativa", 10)
        penalizza(not soup.find("footer"), "❌ Footer mancante", "Nessun riferimento aziendale", 15)
        penalizza(not soup.find(string=lambda x: x and "recension" in x.lower()), "❌ Nessuna recensione visibile", "Manca prova sociale", 15)
        penalizza(not soup.find("h1"), "⚠️ Manca H1", "Struttura poco chiara", 5)
        penalizza(not soup.find(string=lambda x: x and "chi siamo" in x.lower()), "⚠️ Nessuna sezione 'Chi siamo'", "Non trasmette identità", 10)
        penalizza(not soup.find(string=lambda x: x and ("acquista" in x.lower() or "compra" in x.lower())), "⚠️ Nessuna CTA trovata", "L’utente non è guidato", 10)
        penalizza(not soup.find(string=lambda x: x and "blog" in x.lower()), "⚠️ Nessun blog o contenuto", "Manca contenuto per SEO", 5)
        penalizza(not soup.find(string=lambda x: x and "garanzia" in x.lower()), "⚠️ Nessuna garanzia o badge fiducia", "Manca rassicurazione per l’utente", 10)
